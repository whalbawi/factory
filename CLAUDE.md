...
prototyping, setup, build, QA, security, deploy, monitor, and retro. Each phase is a
...
- **`/factory` orchestrator**: Pipeline sequencing, state management in
  ...
  interruption. Supports both forward progression and backward jumps to revisit earlier
  phases.
...
- **`/monitor`** (v1.1): Telemetry review, error triage, health assessment. Output:
  `MONITOR-REPORT.md`.
- **`/retro`** (v1.1): Team retrospective (NOT code review). Standalone, not in the linear
  pipeline. Output: `RETRO-{date}.md`.
...
- Full pipeline orchestration from idea to deployed product (`/factory`):
  `/ideation` -> `/spec` -> `/prototype` -> `/setup` -> `/build` -> `/qa` -> `/security`
  -> `/deploy`
...
- Team retrospective/sync skill for coordination (v1.1: `/retro`)
